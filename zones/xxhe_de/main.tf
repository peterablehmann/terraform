...
resource "hetznerdns_zone" "xxhe_de" {
  name = "xxhe.de"
  ttl  = 60
}
...
resource "hetznerdns_record" "MX_xxhe_de" {
  zone_id = hetznerdns_zone.xxhe_de.id
  name    = "@"
  value   = "10 www208.your-server.de."
  type    = "MX"
}

resource "hetznerdns_record" "TXT_default2307__domainkey_xxhe_de" {
  zone_id = hetznerdns_zone.xxhe_de.id
...
resource "hetznerdns_record" "TXT_SPF_xxhe_de" {
  zone_id = hetznerdns_zone.xxhe_de.id
  name    = "@"
  value   = "\"v=spf1 mx ~all\""
  type    = "TXT"
...
resource "hetznerdns_record" "TXT__dmarc_xxhe_de" {
  zone_id = hetznerdns_zone.xxhe_de.id
  name    = "_dmarc"
...
resource "hetznerdns_record" "SRV__autodiscover__tcp_xxhe_de" {
  zone_id = hetznerdns_zone.xxhe_de.id
...
resource "hetznerdns_record" "SRV__imaps__tcp_xxhe_de" {
  zone_id = hetznerdns_zone.xxhe_de.id
  name    = "_imaps._tcp"
...
resource "hetznerdns_record" "SRV__pop3s__tcp_xxhe_de" {
  zone_id = hetznerdns_zone.xxhe_de.id
  name    = "_pop3s._tcp"
...
resource "hetznerdns_record" "SRV__submission__tcp_xxhe_de" {
  zone_id = hetznerdns_zone.xxhe_de.id
...
resource "hetznerdns_record" "CNAME_autoconfig_xxhe_de" {
  zone_id = hetznerdns_zone.xxhe_de.id
  name    = "autoconfig"
